...
dataset_dir = "/home/ubuntu/progres-main/scripts/dataset"
# update to correct path
coord_dir = "/home/ubuntu/training_coords"
...
dict_filename = f"train_{holdout}.txt"
# Have the dictionary used in training either be randomly generated, or use a preexisting one. 
# random_fams = False

# if random_fams:
#     # Make subset of fam_to_domids randomly
#     fam_to_domids = dict(random.sample(fam_to_domids.items(), 100))
#     cur_time = time.time()
#     dict_filename = f'{cur_time}_fam_to_domids_100.txt'
#     write_dict_to_file(fam_to_domids, dict_filename)
# else:
#     # Overwrite fam_to_domids with previously made family dictionary
#     fam_to_domids = defaultdict(list)
#     with open('/home/ubuntu/progres-main/scripts/1732132951.8092673_fam_to_domids_100.txt') as f:
#         for line in f.readlines():
#             domid, fam = line.rstrip().split()
#             domids_train.append(domid)
#             fam_to_domids[fam].append(domid)
#     dict_filename = '1732132951.8092673_fam_to_domids_100.txt' 
...
## CP mode 
    ...
    # Permute the coordinates
        i = np.random.randint(0, n_res + 1)
        coords_cp = torch.roll(coords, i, 0)
    
    # Change CP invariant mode #
    if CP == False:
        coords_cp = coords

    if gaussian_noise is not None and gaussian_noise > 0:
        coords_cp = coords_cp + torch.randn(coords_cp.shape) * gaussian_noise
    dmap = torch.cdist(coords_cp.unsqueeze(0), coords_cp.unsqueeze(0),
    ...
    if use_tau_angle:
        ...
        vec_ab = coords_cp[1:-2] - coords_cp[ :-3]
        vec_bc = coords_cp[2:-1] - coords_cp[1:-2]
        vec_cd = coords_cp[3:  ] - coords_cp[2:-1]
    ...
    data = Data(x=x, edge_index=edge_index, coords=coords_cp)
    #print(norm_degrees.device, termini_feats.device, taus_pad.device, pos_embed.device, coords_cp.device)
    return data
####
...
#trained_model_fp = '/home/ubuntu/miniforge3/envs/prog/lib/python3.9/site-packages/progres/trained_models/v_0_2_0/trained_model.pt'
#loaded_model = torch.load(trained_model_fp, map_location=device)
#f = os.path.basename(trained_model_fp)
#print('Using model:', f)
...
if os.path.isfile(log_file):
    ...
    report(f'Running in CP invariant mode:{CP}')
    report(f'Using {dict_filename} for family selection')
    report(f'Using {contact_dist} angstroms for contact distance cutoff')
...
for epoch_n in range(starting_epoch_n, n_epochs + 1):
    ...
    for bi, batch in enumerate(train_loader_cont):
        # print out batch index every 100 batches
        if bi % 100 == 0:
            print(bi, "/", len(train_loader_cont))
        # Run batch through model and append the embeddings to batch_embeddings
        out = model(batch.to(device))
        batch_embeddings.append(out)
        # Once 6 batches have been processed (36 % 6 == 0) concatenate all the batch embeddings and calculate the loss
        if (bi + 1) % (n_contrastive // model_batch_train) == 0:
            #concat the 6 batch embeddings 
            feats = torch.cat(batch_embeddings, dim=0).unsqueeze(1)
            #calculate loss based on the 36 embeddings and labels
            loss = loss_fn(feats, labels_contrastive)
            loss.backward()
            train_loss_sum += loss.item()
            # Reset batch embeddings to an empty list
            batch_embeddings = []
        # After 6 batches, update parameters based on gradients using optimizer
    ...
    }, os.path.join(out_dir, f"model_epoch_{epoch_n}.pt"))
    ...
    if sens_seen_mean > best_sens_seen_mean:
        ...
        copyfile(os.path.join(out_dir, f"model_epoch_{epoch_n}.pt"), os.path.join(out_dir, "best_seen_model.pt"))
    if sens_unseen_mean > best_sens_unseen_mean:
        ...
        copyfile(os.path.join(out_dir, f"model_epoch_{epoch_n}.pt"), os.path.join(out_dir, "best_unseen_model.pt"))
