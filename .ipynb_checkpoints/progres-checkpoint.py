...
pre_embedded_dbs = ["scope95", "scope40", "cath40", "ecod70", "pdb100", "af21org", 
                    "ark_scope40_Progres_embed_1_7_25", "ark_scope40_CIRPIN_embed_1_7_25",]
...
trained_model_fp = os.getenv("TRAINED_MODEL_FP", trained_model_fp)
chainsaw_dir      = os.path.join(data_dir, "chainsaw", "model_v3")
chainsaw_model_fp = os.path.join(chainsaw_dir, "weights.pt")

#trained_model_fp = '/home/ubuntu/progres-main/scripts/training/training_11_21_24_CP_100_families/model.pt'
#trained_model_fp= '/home/ubuntu/progres-main/scripts/training/CIRPIN_model/model_5k_cp_epoch301.pt'
#print(trained_model_fp)
#/home/ubuntu/miniforge3/envs/prog/lib/python3.9/site-packages/progres/trained_models/v_0_2_0/trained_model_11_21_24_epoch_969.pt
...
class Model(torch.nn.Module):
    def __init__(self, return_feats=False, return_l2_norm=False):
        super().__init__()
        self.return_l2_norm = return_l2_norm
    ...
    def forward(self, data):
        ...
        # Takes x and coords from data (a graph) and assigns to feats and coords respectively
        ...
        # Encode the node features (which consists of norm_degrees, term_features, taus_pad, pos_embed, but 64/68 of these dimensions is just the pos embed)
        feats = self.node_enc(feats)
        for layer in self.layers:
            # Pass feats and coords through several EGNN layers
            feats, coords = layer(feats, coords, mask, adj_mat)
        # Decode the node feats to node level embeddings
        # feats now represents tensor of node level embeddings
        ...
        l2_norm = torch.norm(out, p=2, dim=1)
        if self.return_l2_norm:
            ## Return both the output embedding vector and the l2norm 
            return out, l2_norm
...
def read_all_atom_coords(fp, fileformat="guess", res_range=None):
...
def embedding_similarity(emb_1, emb_2):
    cosine_dist = (emb_1 * emb_2).sum(dim=-1) # Normalised in the model --> Need to normalize for the node level embeddings since this is NOT normalized in the model
    return (1 + cosine_dist) / 2 # Runs 0 (far) to 1 (close)

def node_embedding_similarity(emb_1, emb_2):
    ### normalize emb1 and emb2
    emb_1 = emb_1 / torch.norm(emb_1, dim=-1, keepdim=True)
    emb_2 = emb_2 / torch.norm(emb_2, dim=-1, keepdim=True)
    ###
    cosine_dist = (emb_1 * emb_2).sum(dim=-1) 
    return (1 + cosine_dist) / 2 # Runs 0 (far) to 1 (close)


def load_trained_model(device="cpu", trained_model=trained_model_fp, return_feats=False, return_l2_norm=False):
    model = Model(return_feats=return_feats, return_l2_norm=return_l2_norm).to(device)
    trained_model_basename = os.path.basename(trained_model)
    print(f'Using model {trained_model_basename}')
    loaded_model = torch.load(trained_model, map_location=device)
    model.load_state_dict(loaded_model["model"])
    model.eval()
    return model

def embed_graph(graph, device="cpu", model=None, return_feats=False, return_l2_norm=False):
    with torch.no_grad():
        if model is None:
            model = load_trained_model(device, trained_model_fp, return_feats, return_l2_norm)
...
def embed_graph_l2_norm(graph, device="cpu", model=None, return_feats=False, return_l2_norm=True):
    '''Return both the embedding of the graph (unnormalized) and the L2 norm'''
    with torch.no_grad():
        if model is None:
            model = load_trained_model(device, trained_model_fp, return_feats, return_l2_norm)
        data_loader = DataLoader([graph], batch_size=1)
        for batch in data_loader:
            emb, emb_l2_norm = model(batch.to(device))
            break
        return emb.squeeze(0), emb_l2_norm
    
def embed_nodes(graph, device="cpu", model=None, return_feats=True, return_l2_norm=False):
    '''Return the node level embeddings for a graph
    return_feats changes what is return from Model() 
    return_feats=True returns 'feat' which are node level embeddings
    return_feats=False returns graph embeddings; use the embed_graph function for this'''
    with torch.no_grad():
        if model is None:
            model = load_trained_model(device, trained_model_fp, return_feats, return_l2_norm)
        data_loader = DataLoader([graph], batch_size=1)
...
def progres_search_generator(querystructure=None, querylist=None, queryembeddings=None,
                             ...
                             batch_size=None, trained_model=trained_model_fp):
    ...
    model = load_trained_model(device, trained_model)
...
def progres_search(querystructure=None, querylist=None, queryembeddings=None, targetdb=None,
                   ...
                   chainsaw=False, device="cpu", batch_size=None, trained_model=trained_model_fp):
...
def progres_search_print(querystructure=None, querylist=None, queryembeddings=None, targetdb=None,
    ...
    for rd in generator:
        ...
        print()
        ### Saving similarity scores to compare results from one model against another
        
        #simscores = rd['similarities']
        #domains = rd['domains']
        #domain_similarity_array = np.array(list(zip(domains, simscores)), dtype=object)
        #np.save('domain_similarity_array.npy', domain_similarity_array)

        

def progres_score(structure1, structure2, fileformat1="guess", fileformat2="guess", device="cpu", trained_model=trained_model_fp):
...
def progres_score_print(structure1, structure2, fileformat1="guess",
                        fileformat2="guess", device="cpu", trained_model=trained_model_fp):
    score = progres_score(structure1, structure2, fileformat1, fileformat2, device, trained_model)
    print(score)

def progres_embed(structurelist, outputfile, fileformat="guess", device="cpu",
                  batch_size=None, float_type=torch.float16, trained_model=trained_model_fp):
    #download_data_if_required()
...
def progres_embed_foldcomp(structurelist, outputfile, fileformat="guess", device="cpu",
                  batch_size=None, float_type=torch.float16, trained_model=trained_model_fp):
    #download_data_if_required()
